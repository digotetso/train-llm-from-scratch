...
- Pilot decision: approved from the exact smoke checkpoint lineage and finite stability evidence. The first pilot command stopped before applying checkpoint state because the immutable run fingerprint contained the smoke commit while the notebook had pulled a newer documentation commit. No pilot update occurred and `latest.pt` remained at step 25. Structured validation now requires matching configuration, tokenizer, dataset, and parameter-count identity while preserving the original commit as provenance. Pilot must be rerun; runtime evaluation, full training, and final summaries remain unobserved.
