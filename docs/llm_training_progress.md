...
- Current phase: the first real 8M TinyStories T4 run is complete; work now moves to final-model analysis, evaluation hardening, and course production.
...
- Clean local verification on 2026-07-20 observes 191 passing tests, including synchronized CUDA benchmark timing and seeded checkpoint evaluation regressions.
...
- Pilot evaluation passed. Both checkpoints loaded, standalone validation loss was `2.9459` with perplexity `19.03`, and `resume_verification.json` confirmed exact step 306, 10,027,008 processed tokens, zero skipped updates, and the unchanged 6,104-step schedule. Corrected pilot artifacts both recorded `evaluation_seed: 42` and matched exactly, so promotion to `full` was approved.
- Full result: passed at exact step 6,104 with 200,015,872 processed token positions. All 651 metric rows were unique and finite, step and token counts were monotonic and consistent, zero optimizer updates were skipped, peak allocated memory remained `951.9` MiB, and the final gradient scale was `524,288`. Training loss reached `1.6116`; validation loss improved from `9.0790` initially to `1.7085` at the final step, with a lowest training-time observation of `1.6953` at step 5,188. Cumulative tracked training time was `2,381.3` seconds.
- Final evaluation passed. With `evaluation_seed: 42`, `latest.pt` measured validation loss `1.7152` and perplexity `5.56`; `best.pt` measured loss `1.7315` and perplexity `5.65`. Both produced substantially more coherent stories than the pilot, while still showing repetition and occasional semantic inconsistency. `latest.pt` is the selected final model because it won the fair same-batch standalone comparison by about `0.94%` in loss. The `best.pt` name reflects the lowest sampled validation result observed during training, not the winner of this final fixed-seed comparison.
...
- **Verified from the complete real run:** checkpoint resume, finite improving learning curves, exact completion at step 6,104, 200,015,872 processed token positions, zero skipped updates, durable Drive artifacts, both checkpoint evaluations, and complete read-only resume verification.
- **Engineering follow-up:** make periodic checkpoint selection use a fixed validation sample or a larger deterministic validation sweep. The completed run remains valid because its final same-seed comparison evaluated both checkpoints fairly and selected `latest.pt`.
...
The next work is evidence-led analysis and teaching from the completed run. Remaining theory is learned just in time while inspecting the relevant code, metrics, checkpoints, and generated samples. The [course outline](../course/outline.md) remains the canonical sequence for lessons on checkpoints, Colab preparation, training, evaluation, debugging, scaling, and teaching.

The repository now has both local verification and real TinyStories T4 evidence. This does not establish a BabyLM run, exact CUDA determinism, broad downstream capability, or production readiness beyond the recorded 8M base-model experiment.
