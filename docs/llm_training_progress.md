...
- Evaluation decision: approved. The next required stage is `evaluate`, which must inspect both `latest.pt` and `best.pt`, verify complete resume state without an optimizer update, persist `resume_verification.json`, generate `run_summary.md`, and expose samples. Full training and final evaluation remain unobserved and unapproved.
