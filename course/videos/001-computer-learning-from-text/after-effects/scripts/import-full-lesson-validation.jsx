...
(function () {
    ...
    function backupExistingTarget() {
        ...
        if (
            ...
            target.name !== "Video001-Exporter-Full-Lesson.aep" ||
            target.alias === true
        ) {
            throw new Error(
    ...
    function moveFailedTargetAside() {
        var failedName;
        var failed;
        if (!target.exists || target.alias === true) {
            return;
        }
        failedName =
            "Video001-Exporter-Full-Lesson.failed-" +
            timestamp() + ".aep";
        failed = new File(target.parent.fsName + "/" + failedName);
        if (failed.exists || !target.rename(failedName)) {
            throw new Error(
